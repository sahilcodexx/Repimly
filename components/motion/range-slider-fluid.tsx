...
export function FluidSlider({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex h-9 w-full touch-none overflow-hidden rounded-full bg-muted",
        TOUCH_GESTURE_CLASS,
        options.disabled
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-between px-3.5 text-xs font-medium text-foreground">
        {row}
      </div>
      ...
      <motion.div className="absolute inset-0" style={{ clipPath }}>
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-between px-3.5 text-xs font-medium text-background">
          {row}
        </div>
      </motion.div>
